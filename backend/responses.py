...
INVALID_PASSWORD = "비밀번호 불일치" # 회원가입 시 "비밀번호"와 "비밀번호 확인" 내용이 다를 경우 : 400 Bad request
...
PASSWORD_MISMATCH = "비밀번호를 확인해 주세요" # 로그인시 이메일은 존재하지만 비밀번호가 일치하지 않을 경우 : 400 Bad request

PRODUCT_MISSING = "상품을 담아주세요" # 결제/장바구니 진행 시 물건을 선택하지 않았을 때

DUPLICATED_INPUT = "이미 존재하는 값입니다" # 중복을 허용하지 않는데 확인한 값이 이미 존재할 경우 : 400 Bad request ex) 회원가입 시 이메일, 셀러명, 브랜드명 등

LOGIN_REQUIRED = "로그인이 필요합니다" # token이 필요한 기능에 token이 없을 시 : 401 Unauthorized

ACCESS_DENIED = "접근 권한이 없습니다" # master 전용 기능에 user나 seller의 토큰 혹은 seller의 전용 기능에 user의 token으로 접속할 시 : 403 Forbidden
